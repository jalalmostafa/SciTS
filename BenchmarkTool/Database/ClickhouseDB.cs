    ...
    public class ClickhouseDB : IDatabase
    {
        ...
        public Task<QueryStatusRead> OutOfRangeQuery(OORangeQuery query)
        {
            ...
            try
            {
                ...
                return Task.FromResult(new QueryStatusRead(true, points, new PerformanceMetricRead(sw.Elapsed.TotalMicroseconds, points, 0, query.StartDate, query.DurationMinutes, 0, Operation.OutOfRangeQuery)));
            }
            catch (Exception ex)
            {
                Log.Error(String.Format("Failed to execute Out of Range Query on ClickhouseDB. Exception: {0}", ex.ToString()));
                return Task.FromResult(new QueryStatusRead(false, 0, new PerformanceMetricRead(0, 0, points, query.StartDate, query.DurationMinutes, 0, Operation.OutOfRangeQuery), ex, ex.ToString()));
            }
        }
        ...
        public Task<QueryStatusRead> RangeQueryAgg(RangeQuery query)
        {
            ...
            try
            {
                ...
                return Task.FromResult(new QueryStatusRead(true, points, new PerformanceMetricRead(sw.Elapsed.TotalMicroseconds, points, 0, query.StartDate, query.DurationMinutes, _aggInterval, Operation.RangeQueryAggData)));
            }
            catch (Exception ex)
            {
                Log.Error(String.Format("Failed to execute Range Query Agg Data on ClickhouseDB. Exception: {0}", ex.ToString()));
                return Task.FromResult(new QueryStatusRead(false, 0, new PerformanceMetricRead(0, 0, points, query.StartDate, query.DurationMinutes, _aggInterval, Operation.RangeQueryAggData), ex, ex.ToString()));
            }
        }

        public Task<QueryStatusRead> RangeQueryRaw(RangeQuery query)
        {
            int points = 0;
            try
            {
                ...
                return Task.FromResult(new QueryStatusRead(true, points, new PerformanceMetricRead(sw.Elapsed.TotalMicroseconds, points, 0, query.StartDate, query.DurationMinutes, 0, Operation.RangeQueryRawData)));
            }
            catch (Exception ex)
            {
                Log.Error(String.Format("Failed to execute Range Query Raw Data on ClickhouseDB. Exception: {0}", ex.ToString()));
                return Task.FromResult(new QueryStatusRead(false, 0, new PerformanceMetricRead(0, 0, points, query.StartDate, query.DurationMinutes, 0, Operation.RangeQueryRawData), ex, ex.ToString()));
            }
        }

        public Task<QueryStatusRead> RangeQueryRawAllDims(RangeQuery query)
        {
            int points = 0;
            try
            {
                ...
                return Task.FromResult(new QueryStatusRead(true, points, new PerformanceMetricRead(sw.Elapsed.TotalMicroseconds, points, 0, query.StartDate, query.DurationMinutes, 0, Operation.RangeQueryRawAllDimsData)));
            }
            catch (Exception ex)
            {
                Log.Error(String.Format("Failed to execute Range Query Raw Data on ClickhouseDB. Exception: {0}", ex.ToString()));
                return Task.FromResult(new QueryStatusRead(false, 0, new PerformanceMetricRead(0, 0, points, query.StartDate, query.DurationMinutes, 0, Operation.RangeQueryRawAllDimsData), ex, ex.ToString()));
            }
        }

        public Task<QueryStatusRead> RangeQueryRawLimited(RangeQuery query, int limit)
        {
            int points = 0;
            try
            {
        ...
        public Task<QueryStatusRead> RangeQueryRawAllDimsLimited(RangeQuery query, int limit)
        {
            int points = 0;
            try
            {
        ...
        public Task<QueryStatusRead> AggregatedDifferenceQuery(ComparisonQuery query)
        {
            ...
            try
            {
                ...
                reader.ReadAll(rowReader => { points++; });
                sw.Stop();
                return Task.FromResult(new QueryStatusRead(true, points, new PerformanceMetricRead(sw.Elapsed.TotalMicroseconds, points, 0, query.StartDate, query.DurationMinutes, _aggInterval, Operation.DifferenceAggQuery)));
            }
            catch (Exception ex)
            {
                ...
                return Task.FromResult(new QueryStatusRead(false, 0, new PerformanceMetricRead(0, 0, points, query.StartDate, query.DurationMinutes, _aggInterval, Operation.DifferenceAggQuery), ex, ex.ToString()));
            }
        }
        ...
        public Task<QueryStatusRead> StandardDevQuery(SpecificQuery query)
        {
            ...
            try
            {
                ...
                reader.ReadAll(rowReader => { points++; });
                sw.Stop();
                return Task.FromResult(new QueryStatusRead(true, points, new PerformanceMetricRead(sw.Elapsed.TotalMicroseconds, points, 0, query.StartDate, query.DurationMinutes, 0, Operation.STDDevQuery)));
            }
            catch (Exception ex)
            {
                Log.Error(String.Format("Failed to execute STD Dev query on ClickhouseDB. Exception: {0}", ex.ToString()));
                return Task.FromResult(new QueryStatusRead(false, 0, new PerformanceMetricRead(0, 0, points, query.StartDate, query.DurationMinutes, 0, Operation.STDDevQuery), ex, ex.ToString()));
            }
        }
        ...
        public Task<QueryStatusWrite> WriteRecord(IRecord record)
        {
            try
            {
                ...
                command.ExecuteNonQuery();
                sw.Stop();
                return Task.FromResult(new QueryStatusWrite(true, new PerformanceMetricWrite(sw.Elapsed.TotalMicroseconds, 1, 0, Operation.StreamIngestion)));
            }
            catch (Exception ex)
            {
                Log.Error(String.Format("Failed to insert batch into Clickhouse. Exception: {0}", ex.ToString()));
